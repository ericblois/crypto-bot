...
def run_test(num_prev_intervals, bot_values, hold_values, bot_to_hold_ratios, all_trade_profits, all_trade_losses):
    for j in range(int(num_prev_intervals / 1440)):
        # --- VALUE VARIABLES ---
        ...
        short_gain_rma = 0
        short_loss_rma = 0
        longest_ema = data_vector[j * 1440][0]
        long_ema = data_vector[j * 1440][0]
        short_ema = data_vector[j * 1440][0]
        macd_increases = 0
        macd_decreases = 0
        last_macd = 0
        macd_limit = 6
        last_long_ema = data_vector[j * 1440][0]
        last_short_ema = data_vector[j * 1440][0]
        last_price = 1
        max_price = 999999
        min_price = 0
        time_since_buy = 0
        ...
        for i, row in enumerate(data_vector[j * 1440: (j + 1) * 1440]):
            '''
            vol = row[13]
            MA_3 = row[6]
            MA_6 = row[5]
            ...
            short_ema_period = 12
            ema_smoothing = 2
            longest_ema = price * (ema_smoothing / (1 + longest_ema_period)) + longest_ema * (
                        1 - (ema_smoothing / (1 + longest_ema_period)))
            long_ema = price * (ema_smoothing / (1 + long_ema_period)) + long_ema * (
                        1 - (ema_smoothing / (1 + long_ema_period)))
            ...
            # Buys
            if not bought and not buy_trig and (price <= bb_lower_band and short_rsi < 35 and price < longest_ema):
                buy_trig = True
                short = False
            if not bought and not buy_trig and (price >= bb_upper_band and short_rsi > 65 and price > longest_ema):
                buy_trig = True
                short = True
            # Sells
            ...
            if (buy_trig and not short and short_rsi >= 35 and price > last_price) or (
                    buy_trig and short and short_rsi <= 65 and price < last_price):
            ...
            elif (sell_trig and ((not short and short_rsi < max_rsi - 3) or (short and short_rsi > min_rsi + 3))):
                if (not short and price < max_price * (1 - stop_loss)) or (
                        short and price > min_price * (1 + stop_loss)):
                    stop_count += 1
                # Keep track of balances and prices
                sell_price = price
                if not short:
...
if __name__ == "__main__":
    ...
    run_test(6 * 20160, BOT_VALUES, HOLD_VALUES, BOT_TO_HOLD_RATIOS, ALL_TRADE_PROFITS, ALL_TRADE_LOSSES)
    ...
    print("------\nMedian Bot Value: " + str(round(median_bot_value, 2)) + "% | Mean Bot Value: " + str(
        ...
        round(WIN_COUNT / ROUND_COUNT * 100, 2)) + "%)"
          + "\nTotal: $" + str(round(np.prod([value / 100 for value in BOT_VALUES]) * 100, 2)) + "(" + str(
        round(np.prod([value / 100 for value in BOT_VALUES]) * 100 - 100, 2)) + "%)"
          + "\n-----"
